...
TMP_DOWNLOAD_PATH="$TMP_DIR/cover_downloading.png"
...
ITUNES_URL=$(curl -s "https://itunes.apple.com/search?term=$SEARCH_TERM_ITUNES&entity=song&limit=1" | jq -r '.results[0].artworkUrl100' 2>/dev/null)

if [[ -n "$ITUNES_URL" && "$ITUNES_URL" != "null" && "$ITUNES_URL" != "" ]]; then
    # Solicitar un tamaño optimizado de 400x400
    OPTIMIZED_URL=$(echo "$ITUNES_URL" | sed 's/100x100/400x400/')
    if curl -s -o "$TMP_DOWNLOAD_PATH" "$OPTIMIZED_URL" && [[ -s "$TMP_DOWNLOAD_PATH" ]]; then
        mv "$TMP_DOWNLOAD_PATH" "$TMP_COVER_PATH"
        exit 0
    fi
fi
...
if [[ -n "$MPRIS_URL" ]]; then
    if [[ "$MPRIS_URL" == "http"* ]]; then
        curl -s -o "$TMP_DOWNLOAD_PATH" "$MPRIS_URL"
